 ...
 * 
 */
public class SsqRecordServiceImpl extends BaseServiceImpl implements SsqRecordService {
...
//			getSsqRecordDao().deleteByHql("delete from SsqRecord");
	...
	public void writeNotExistSsqStatsByNow(String typeName) {
		if(typeName.equals(com.wzm.util.SsqStatsType.LAST_SAME_STATS)) {
			// 获取formSsqIndex，即要写入的统计表信息的开始期
			String hql = "select max(ssqIndex) from SsqLastSameStats";
			...
			writeSsqLastSameStats(fromSsqIndex, toSsqIndex);
		} else {
	...
	@Override
	public Map<String, Object[]> testRed(int ssqIndex, List<String> formulas) {
		Map<String, Object[]> result = new HashMap<String, Object[]>();
		
		Map<String, Interpreter> expMap = new HashMap<String, Interpreter>();
		
		List<SsqRecord> list = getSsqRecordDao().findSsqRecordsByHql(" from SsqRecord where ssqIndex >= 2010001 and ssqIndex <= ? order by ssqIndex", new Integer[]{ssqIndex} );
		...
		result.put("summary", new Object[]{1,summaryList});
	...
	@Override
	public Map<String, Object[]> testBlue(int ssqIndex, List<String> formulas) {
		Map<String, Object[]> result = new HashMap<String, Object[]>();
	...
	@Override
	public void writeAllSsqColRowStats() {
		writeAllSsqStats("colRowStats");
	}

	@Override
	public void writeSsqColRowStats(int fromSsqIndex, int toSsqIndex) {
		writeSsqStats("colRowStats", fromSsqIndex, toSsqIndex);
	}

	@Override
	public void writeAllSsqLastSameStats() {
		int ssqIndex1 = getSsqRecordDao().getFunctionIntValue("select min(ssqIndex) from SsqRecord");
		int ssqIndex2 = getSsqRecordDao().getFunctionIntValue("select max(ssqIndex) from SsqRecord");
		writeSsqLastSameStats(ssqIndex1, ssqIndex2);
	}

	@Override
	public void writeSsqLastSameStats(int fromSsqIndex, int toSsqIndex) {
		// 1.删除开始期和结束期范围内的统计表
		getSsqRecordDao().deleteByHql(
				"delete from SsqLastSameStats a where a.ssqIndex>=? and a.ssqIndex<=?",
				new Object[] { fromSsqIndex, toSsqIndex });

		// 2.取出双色球开奖记录集合
		List<SsqRecord> records = getSsqRecordDao().findSsqRecordsByIndexScope(
				fromSsqIndex-1, toSsqIndex);
		...
		for (int i=1;i<records.size();i++) {
			...
			SsqRecord lastRecord = records.get(i-1);
			
			SsqLastSameStats stats = new SsqLastSameStats();

			// 计算统计值
			stats.buildStats(record, lastRecord);
